...
```mermaid
flowchart TD
    A[CloudFrontディストリビューションを削除] --> B[OACを削除]
    B --> C[S3バケットを空にする]
    C --> D[S3バケットを削除]
    D --> E[全リソース削除完了]
```
...
### ステップ2: OAC（Origin Access Control）を削除する

1. AWSコンソール → **CloudFront** → 左メニュー「オリジンアクセス」→「コントロール設定」

2. 対象のOAC（`バケット名-oac`）を選択

3. 「削除」をクリックして削除

---

### ステップ3: S3バケットを空にして削除する
